defmodule CommunicationServer do
  ...
  def sendCom(port, message) do
    GenServer.cast({:local, port}, {:send, message})
  end

  def accept(port) do
    GenServer.cast({:local, port}, :accept)
  end
  ...
  def handle_cast({:send, message}, {socket, _opts}) do
  ...
  def handle_cast(:accept, {socket, _opts}) do
